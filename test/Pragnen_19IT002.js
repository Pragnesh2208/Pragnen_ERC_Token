...
contract('Pragnen_19IT002', function(accounts) {
  var tokenInstance;

  it('initializes the contract with the correct values', function() {
    return Pragnen_19IT002.deployed().then(function(instance) {
      tokenInstance = instance;
      return tokenInstance.name();
    }).then(function(name) {
      assert.equal(name, 'Pragnen_19IT002', 'has the correct name');
      return tokenInstance.symbol();
    }).then(function(symbol) {
      assert.equal(symbol, '19IT002', 'has the correct symbol');
      return tokenInstance.standard();
    }).then(function(standard) {
      assert.equal(standard, 'Pragnen_19IT002 v1.0', 'has the correct standard');
    });
  })
...
it("transfers token ownership", function() {
    return Pragnen_19IT002.deployed()
